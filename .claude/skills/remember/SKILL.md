...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope irc-lens
...
scope:** the default personal scope is **private** (`--scope irc-lens
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `irc-lens`). Private records are served only to a recall in the same
